...
export class AssetService {
  constructor(private dbConnectorService: DbConnectorService, private gameGateway: GameGateway) {}

  async updateAsset(gameId: string, assetId: string, playerId: string, rating: number): Promise<Asset> {
  ...
  createAsset(newAsset: CreateAsset): Asset {
    return new Asset(uuidv4(), newAsset.gameId, newAsset.name, {});
  }

  async saveAndPublishAsset(asset: Asset): Promise<Asset> {
